"""
ZP HIGHER LITE — PySide6 UI
Reconstructed from ui.dll. Three-column layout: Shooting | Features | System.
"""
import sys
import os
import json
...
class ShootingColumn(QWidget):
    ...
    def __init__(self):
        ...
        self.rt_minus2  = StepBtn("-2");  self.rt_minus05 = StepBtn("-.5")
        self.rt_plus05  = StepBtn("+.5"); self.rt_plus2   = StepBtn("+2")
        self.rt_slider  = CyanSlider(0, 100, self.DEFAULT_RT)
        for w in (self.rt_minus2, self.rt_minus05):   row1.addWidget(w)
        row1.addWidget(self.rt_slider, 1)
        for w in (self.rt_plus05, self.rt_plus2):     row1.addWidget(w)
        lay.addLayout(row1)

        hint1 = QHBoxLayout()
        hint1.addWidget(SmallLabel("TAKE 4 SHOTS (HOLD, DON'T RELEASE)", TEXT_DIM, 7), 1)
        self.rt_recal = RecalBtn(); hint1.addWidget(self.rt_recal)
        lay.addLayout(hint1)

        lay.addSpacing(10); lay.addWidget(HRule()); lay.addSpacing(10)
        ...
        self.l2_minus2  = StepBtn("-2");  self.l2_minus05 = StepBtn("-.5")
        self.l2_plus05  = StepBtn("+.5"); self.l2_plus2   = StepBtn("+2")
        self.l2_slider  = CyanSlider(0, 100, self.DEFAULT_L2)
        for w in (self.l2_minus2, self.l2_minus05):   row2.addWidget(w)
        row2.addWidget(self.l2_slider, 1)
        for w in (self.l2_plus05, self.l2_plus2):     row2.addWidget(w)
        lay.addLayout(row2)

        hint2 = QHBoxLayout()
        hint2.addWidget(SmallLabel("TAKE 4 L2/LT SHOTS (HOLD, DON'T RELEASE)", TEXT_DIM, 7), 1)
        self.l2_recal = RecalBtn(); hint2.addWidget(self.l2_recal)
        lay.addLayout(hint2)
        ...
        trow = QHBoxLayout()
        self.tempo_chk = ToggleCheckBox(); self.tempo_chk.setChecked(True)
        ...
        trow.addWidget(self.tempo_chk); trow.addWidget(self.tempo_state); trow.addStretch()
        lay.addLayout(trow)

        self.tempo_val = BigValue("39ms")
        lay.addWidget(self.tempo_val)

        tslide = QHBoxLayout(); tslide.setSpacing(4)
        self.tempo_minus = StepBtn("-"); self.tempo_minus.setFixedWidth(26)
        self.tempo_slider = CyanSlider(10, 100, self.DEFAULT_TEMPO)
        self.tempo_plus  = StepBtn("+"); self.tempo_plus.setFixedWidth(26)
        tslide.addWidget(self.tempo_minus); tslide.addWidget(self.tempo_slider, 1)
...
class FeaturesColumn(QWidget):
    def __init__(self):
        ...
        lay.addSpacing(12); lay.addWidget(HRule()); lay.addSpacing(12)

        lay.addWidget(SmallLabel("DEFENSE", TEXT_MED, 8))
        def_row = QHBoxLayout()
        self.def_chk = ToggleCheckBox(); self.def_chk.setChecked(True)
        ...
        def_row.addWidget(self.def_chk); def_row.addWidget(self.def_state); def_row.addStretch()
        ...
        ll_row = QHBoxLayout()
        self.ll_chk = ToggleCheckBox(); self.ll_chk.setChecked(True)
        ...
        ll_row.addWidget(self.ll_chk); ll_row.addWidget(self.ll_state); ll_row.addStretch()
        lay.addLayout(ll_row)

        lay.addStretch()
        self._connect()
        self._wire_network_optimizer()
    ...
    def _wire_network_optimizer(self):
        try:
            from core.network_optimizer import apply as _apply, restore as _restore
            self._net_apply   = _apply
            self._net_restore = _restore
            if self.ll_chk.isChecked():
                _apply()
        except Exception:
            self._net_apply   = None
            self._net_restore = None
    ...
    def _ll_changed(self, state):
        ...
        if on and self._net_apply:
            self._net_apply()
        elif not on and self._net_restore:
            self._net_restore()
...
class SystemColumn(QWidget):
    def __init__(self):
        ...
        lay.addSpacing(6); lay.addWidget(HRule()); lay.addSpacing(8)
        ...
        self.lite_radio.setFont(poppins(9)); self.lite_radio.setChecked(True)
        self.lite_radio.setStyleSheet(radio_style)
        self.gpu_radio = QRadioButton("GPU Mode (NVIDIA)")
        self.gpu_radio.setFont(poppins(9)); self.gpu_radio.setStyleSheet(radio_style)
        ...
        lay.addSpacing(8); lay.addWidget(HRule()); lay.addSpacing(8)
        ...
        fps_row.addWidget(fps_name); fps_row.addStretch(); fps_row.addWidget(self.fps_val)
        ...
        rel_row.addWidget(rel_name); rel_row.addStretch(); rel_row.addWidget(self.rel_val)
        ...
        self._get_shooting_state = None
        self._get_features_state = None
    ...
    def _finish_scan(self):
        ...
        try:
            import bettercam
            devices = bettercam.device_info()
            self.gpu_edit.setText(devices[0] if devices else "GPU 0")
        except Exception:
            self.gpu_edit.setText("NVIDIA GeForce RTX 5050")
    ...
    def _toggle_engine(self):
        ...
        if self.running:
            ...
            shoot  = self._get_shooting_state() if callable(self._get_shooting_state) else {}
            feats  = self._get_features_state()  if callable(self._get_features_state) else {}
            config = {**shoot, **feats}
    ...
    def _tick(self):
        if self.current_engine:
            fps   = getattr(self.current_engine, "fps_cur", 0)
            shots = getattr(self.current_engine, "shots_fired", 0)
            self.fps_val.setText(f"{fps:.0f}" if fps else "--")
            self.rel_val.setText(str(shots))
            self.rel_val.setStyleSheet(f"color:{ACCENT};")

    def _pc_changed(self, button_id, checked):
        if not checked: return
        self.pc_hint.setText(
            "Optimised for laptops & PCs without NVIDIA GPU" if button_id == 0
            else "Full GPU acceleration — requires NVIDIA CUDA driver"
        )
    ...
    def set_state(self, data):
        ...
        btn = self.pc_group.button(data.get("pc_mode", 0))
...
class TitleBar(QWidget):
    def __init__(self, parent, shooting, features, system):
        ...
        pipe     = QLabel(" | ")
        pipe.setFont(aquire(14)); pipe.setStyleSheet(f"color:{TEXT_DIM};")
        subtitle = QLabel("PLAYSTATION")
        subtitle.setFont(poppins(9))
        subtitle.setStyleSheet(f"color:{TEXT_MED}; letter-spacing:4px;")
        lay.addWidget(title); lay.addWidget(pipe); lay.addWidget(subtitle)
        lay.addStretch()
        ...
        lay.addWidget(self.save_btn); lay.addSpacing(16)

        for sym, fn in [("─", parent.showMinimized), ("✕", parent.close)]:
            btn = QPushButton(sym)
            btn.setFixedSize(28, 28); btn.setFont(poppins(9))
            btn.setStyleSheet(
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.system._get_features_state = self.features.get_state
    ...
    def resizeEvent(self, event):
        super().resizeEvent(event); self._sync_dots()

    def showEvent(self, event):
        super().showEvent(event); self._sync_dots()
