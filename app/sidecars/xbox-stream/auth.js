...
//
//   node ./auth.js  [--tokens-file=<path>] [--data-dir=<root>] [--account-key=<key>]
//   node ./auth.js  show    [...]   — print cache status
//   node ./auth.js  logout  [...]   — wipe the cache
//
// Resolution rules for the on-disk token file:
//   --tokens-file wins if given.
//   else if --data-dir is given:  <data-dir>/sessions/<accountKey>/xbox-tokens.json
//   else fall back to             <__dirname>/.xbox.tokens.json
// accountKey defaults to "default" — matches main.js back-compat behaviour.
//
...
function parseArgs() {
    ...
    for (const a of process.argv.slice(2)) {
        ...
        if (m) {
            ...
            else if (m[1] === 'data-dir')    out.dataDir    = m[2];
            else if (m[1] === 'account-key') out.accountKey = m[2];
    ...
    if (!out.tokensFile) {
        if (out.dataDir) {
            out.tokensFile = path.join(out.dataDir, 'sessions', out.accountKey, 'xbox-tokens.json');
        } else {
...
async function main() {
    const { cmd, tokensFile, accountKey } = parseArgs();
    console.error(`[auth] cmd=${cmd} accountKey=${accountKey} tokens=${tokensFile}`);
    ...
    if (cmd === 'show') {
        ...
        emit({ type: 'status', method: m, accountKey, tokensFile,
